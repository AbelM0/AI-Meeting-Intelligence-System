...
export function UploadProgress({
  ...
  return (
    <div className="rounded-lg border border-[#bae6fd] bg-[#f0f9ff] p-4" aria-live="polite">
      <div className="flex min-w-0 items-baseline justify-between gap-4">
        <p className="truncate text-sm font-semibold text-[#111827]">{fileName}</p>
        <span className="shrink-0 font-mono text-xs font-semibold tabular-nums text-[#0e7490]">
          {boundedPercentage}%
        </span>
      </div>
      <div
      ...
      >
        <div
          className="h-full rounded-full bg-[#06b6d4] transition-[width] duration-200 ease-out motion-reduce:transition-none"
          style={{ width: `${boundedPercentage}%` }}
        />
      </div>
      <div className="mt-2 flex flex-wrap justify-between gap-x-4 gap-y-1 text-xs text-[#4b5563]">
        <span>{label}</span>
