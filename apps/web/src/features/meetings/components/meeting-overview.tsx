...
function EvidenceLink({ item, onViewTranscript }: Readonly<{ item: Pick<Decision, 'sourceStartTime' | 'sourceSegmentId' | 'sourceSegment'>; onViewTranscript: NavigateToEvidence }>) {
  ...
  return (
    <button type="button" onClick={() => onViewTranscript({ sourceSegmentId: item.sourceSegmentId, sourceStartTime: timestamp })} className="inline-flex h-8 items-center gap-1.5 text-xs font-medium text-primary transition hover:text-primary">
...
function IntelligenceState({ meetingId, children }: Readonly<{ meetingId: string; children: (data: NonNullable<ReturnType<typeof useMeetingIntelligence>['data']>) => React.ReactNode }>) {
  ...
  if (query.isError) return <div className="my-5 border border-destructive/25 bg-destructive/10 p-4 text-destructive" role="alert"><div className="flex gap-3"><WarningCircleIcon className="mt-0.5 h-5 w-5 shrink-0" weight="duotone" aria-hidden="true" /><div><p className="font-semibold">Meeting analysis unavailable</p><p className="mt-1 text-sm">{getApiErrorMessage(query.error, 'The transcript remains available. Retry analysis to regenerate these results.')}</p></div></div></div>;
  ...
  return (
    <li className="grid gap-2 py-3 sm:grid-cols-[minmax(0,1fr)_110px_100px_74px_auto] sm:items-center">
      <div className="flex min-w-0 items-start gap-2">
        <input type="checkbox" checked={completed} disabled={mutation.isPending} onChange={() => mutation.mutate({ actionItemId: item.id, status: completed ? 'OPEN' : 'COMPLETED' })} className="mt-0.5 h-4 w-4 rounded border-input accent-primary" aria-label={`${completed ? 'Reopen' : 'Complete'} action item: ${item.task}`} />
      ...
      {(intelligence) => (
        <div className="grid gap-3 py-5 lg:grid-cols-[minmax(0,1.4fr)_minmax(280px,0.8fr)]">
          <div className="space-y-3">
            <section className="rounded-lg border bg-popover p-4 sm:p-5"><SectionLabel>Summary</SectionLabel><p className="mt-3 max-w-[72ch] whitespace-pre-line text-sm leading-6 text-muted-foreground">{intelligence.summary.overview}</p></section>
