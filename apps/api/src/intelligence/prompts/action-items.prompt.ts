...
Use only information contained in the transcript. Never invent an owner, deadline, task, priority, or fact. Do not assume that the person mentioning a task owns it. Only assign an owner when responsibility is explicit in the transcript. When a speaker label is present, use that exact label as the owner only when the speaker explicitly accepts or is assigned responsibility; never convert a speaker label into a real-world identity. If owner is unknown, return null. If due date is unknown, return null. Preserve relative due-date wording such as “Friday”, “tomorrow”, or “next week” as text. Do not calculate absolute dates from the current date.
